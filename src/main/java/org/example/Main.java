...
public class Main {
    static final int N_MIN = 66;
    static final int N_MAX = 66;
    ...
    static List<SequencePair> findAllPairsDispatcher(int n) {
        ...
        Set<SequencePair> uniquePairs = new LinkedHashSet<>(foundPairs);
    ...
    static List<SequencePair> findPairsByRandomizedBucketing(int n) {
                        ...
                        for (int key : commonKeys) {
                            ...
                            for (SequenceData dataA : listA) {
                                ...
                                for (SequenceData dataB : listB) {
                                    if (criticalErrorOccurred.get() || System.currentTimeMillis() >= deadline ||
                                        (!unlimitedPairs && foundPairsSet.size() >= MAX_UNIQUE_PAIRS_PER_N)) break;
                                    // включить, если нужны и одинаковые пары
                                    // if (dataA.sequence() == dataB.sequence() || Arrays.equals(dataA.sequence(), dataB.sequence())) {
                                    //  continue;
                                    //}
        ...
        try {
            while (latch.getCount() > 0 && System.currentTimeMillis() < deadline && !criticalErrorOccurred.get() && foundPairsSet.size() < MAX_UNIQUE_PAIRS_PER_N) {
        ...
        try {
            if (!executor.awaitTermination(1, TimeUnit.SECONDS)) {
                System.err.println("\nПотоки не завершились вовремя, принудительное завершение.");
                List<Runnable> droppedTasks = executor.shutdownNow();
                if (!droppedTasks.isEmpty()) {
                    System.err.println("Задачи, которые не были выполнены: " + droppedTasks.size());
                }
            }
        } catch (InterruptedException e) {
            System.err.println("\nПрервано ожидание завершения потоков.");
        ...
        if (System.currentTimeMillis() >= deadline && !criticalErrorOccurred.get() && foundPairsSet.size() < MAX_UNIQUE_PAIRS_PER_N) {
    ...
    static long[] cyclicAutocorrelation(long[] seq) {
        ...
        try {
            ...
            if (data == null || data.length < requiredSize) {
                ...
                if (bufferSize == 0 && requiredSize > 0) bufferSize = Integer.highestOneBit(requiredSize) <<1;
                if (bufferSize == 0) bufferSize = 256;
                data = new double[Math.max(requiredSize, bufferSize)];
                dataBufferThreadLocal.set(data);
            } else {
                Arrays.fill(data, 0, Math.min(data.length, requiredSize*2), 0.0);
            }
    ...
    static long[] generateSequenceWithK1(int length, int k1, Random rand) {
        if (k1 < 0 || k1 > length) {
            throw new IllegalArgumentException("k1 должно быть в диапазоне [0, length]");
        }
